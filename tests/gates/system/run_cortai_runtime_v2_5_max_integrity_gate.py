...
def _load_artifacts() -> dict[str, dict[str, Any] | str]:
    ...
    artifacts["master_state_text"] = (ROOT / "docs" / "runtime" / "architecture" / "CORTAI_RUNTIME_MASTER_STATE_V2_5.md").read_text(encoding="utf-8")
    return artifacts


def _run_critical_tests() -> list[dict[str, Any]]:
    commands = [
        [sys.executable, "tests/experiment/test_experiment_capability_phase2_unittest.py"],
        [sys.executable, "tests/agents/script/test_script_generation_unittest.py"],
        [sys.executable, "tests/agents/account_health/test_account_health_agent_phase2_unittest.py"],
        [sys.executable, "tests/attribution/test_content_attribution_phase_d_bounded_integration_unittest.py"],
    ]
