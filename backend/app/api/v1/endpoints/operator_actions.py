...
def pause_rollout(payload: PauseRolloutRequest):
    service = _service()
    return _handle_action(lambda: service.pause_rollout(operator_id=payload.operator_id, reason=payload.reason))
...
def resume_rollout(payload: ResumeRolloutRequest):
    service = _service()
    return _handle_action(lambda: service.resume_rollout(operator_id=payload.operator_id, reason=payload.reason))
...
def requeue_task(payload: RequeueTaskRequest):
    service = _service()
...
def rebuild_event_index(payload: RebuildEventIndexRequest):
    service = _service()
    return _handle_action(lambda: service.rebuild_event_index(operator_id=payload.operator_id, reason=payload.reason))
...
def ack_alert(payload: AckAlertRequest):
    service = _service()
    return _handle_action(
