...
def derive_operational_policy(collector_summary: dict[str, Any] | None) -> dict[str, Any]:
    """
    Deriva um shape de decisao deterministico a partir do collector summary.

    Mantem chaves legadas por compatibilidade temporaria com consumidores
    existentes enquanto o core e desacoplado dos endpoints.
    """
...
def _derive_decision_from_score_and_signals(
    *,
    ...
    """Mapeia score em state/decision e resume sinais de forma deterministica."""
    signals: list[str] = []
...
def derive_policy_bridge(
    ...
    *,
    ...
    """Gera um bloco read-only aditivo para o overview a partir do collector."""
    if not collector_summary:
        return None
