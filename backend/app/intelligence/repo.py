...
from app.intelligence.store_jsonl import append_record, read_all_records
...
def _get_by_key(key_field: str, key_value: str, *, path: Path) -> dict[str, Any] | None:
...
def save_if_absent(record: dict[str, Any], *, key_field: str, path: Path) -> str:
    existing = _get_by_key(key_field, str(record[key_field]), path=path)
    if existing is None:
        append_record(record, path)
        return "WRITTEN"
    if _canonical_payload(existing) == _canonical_payload(record):
        return "NOOP"
    raise IntelligenceConflictError(f"INTELLIGENCE_CONFLICT:{key_field}")
