...
# Warm-up read-path snapshots from host (uses edge by default) and runs the refresh worker inside cortai_api.

BASE_URL="${BASE_URL:-http://localhost:8001}"
API_CONTAINER_NAME="${API_CONTAINER_NAME:-cortai_api}"
...
run_jobs() {
  docker exec -i "$API_CONTAINER_NAME" sh -lc "cd /app && python scripts/run_read_refresh_jobs.py --limit ${JOB_LIMIT}"
}
...
echo "warmup_read_path: base_url=${BASE_URL} days=${DAYS} runs_start=${RUNS_START_DATE} runs_end=${RUNS_END_DATE} job_limit=${JOB_LIMIT}"
